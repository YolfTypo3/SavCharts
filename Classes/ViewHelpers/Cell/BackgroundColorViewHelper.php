...
/**
 * A viewHelper to get the background color of a cell.
 *
 * @package SavCharts
 */
final class BackgroundColorViewHelper extends AbstractSavChartsViewHelper
{
    ...
    {  
        ...
        if ($worksheet !== null)  {
            ...
            return '#' . substr($fillColor, 2, 6) . dechex((int) (hexdec(substr($fillColor, 0, 2))*$alpha/100));
        } else {
            $message = sprintf(
                'The viewHelper <c:cell.backgroundcolor> must be called in inside <c:worksheet>.'
                );
            throw new \InvalidArgumentException($message);
        }  
    }
